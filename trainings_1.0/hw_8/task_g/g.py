...
class BinTree:
    ...
    def get_elements_with_one_child(self) -> List[int]:
        """
        Функция, которая выводит список элементов, у которых только один потомок.
...
def get_tree_elements_with_one_child(input_list: List[int]) -> Tuple[BinTree, List[int]]:
    """
    Функция которая выводит элементы дерева, у которых только один потомок.

    :param input_list: последовательность чисел из которых необходимо построить дерево
    :type input_list: List[int]

    :return: сущность дерева и последовательность элементов дерева, у которых только один потомок
    :rtype: List[int]
    """
    bin_tree = BinTree()
    ...
    return bin_tree, bin_tree.get_elements_with_one_child()


def main():
    """Основная функция для чтения входных данных и вывода результата."""
    input_list = list(map(int, input().split()))
    for element in get_tree_elements_with_one_child(input_list)[1]:
        print(element)
