...
class BinTree:
    ...
    def get_all_elements(self) -> List[int]:
        """
        Функция, которая выводит список элементов в порядке возрастания.
            Возвращаемое значение:
...
def get_tree_elements(input_list: List[int]) -> Tuple[BinTree, List[int]]:
    """
    Функция которая выводит элементы дерева в порядке возрастания.
        Параметры:
            :param input_list: последовательность чисел из которых необходимо построить дерево
            :type input_list: List[int]
        Возвращаемое значение:
            :return: сущность дерева и последовательность элементов дерева
            :rtype: List[int]
    """
    bin_tree = BinTree()
    ...
    return bin_tree, bin_tree.get_all_elements()


def main():
    """Основная функция для чтения входных данных и вывода результата."""
    input_list = list(map(int, input().split()))
    for element in get_tree_elements(input_list)[1]:
        print(element)
